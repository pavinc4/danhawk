// tool_commands.rs
// Generic file read/write + app scanning + icon extraction for tool UIs.
// File operations are scoped to the INSTALLED tool folder in AppData.
// Never reads/writes to the source tools-repo/ folder.
...
// Resolves a safe path inside the installed tool folder.
// Blocks path traversal attempts (../../etc).
fn safe_tool_path(tool_id: &str, filename: &str) -> Result<PathBuf, String> {
    let base = paths::installed_tool_dir(tool_id);
...
/// Read a text file from the installed tool folder.
#[tauri::command]
pub fn tool_read_file(tool_id: String, filename: String) -> Result<String, String> {
    let path = safe_tool_path(&tool_id, &filename)?;
    if !path.exists() { return Ok(String::new()); }
    fs::read_to_string(&path).map_err(|e| {
        logger::warn(&format!("[tool-cmd] read failed {}: {}", path.display(), e));
        e.to_string()
    })
}

/// Write a text file to the installed tool folder.
#[tauri::command]
pub fn tool_write_file(tool_id: String, filename: String, content: String) -> Result<(), String> {
    let path = safe_tool_path(&tool_id, &filename)?;
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| e.to_string())?;
    }
    fs::write(&path, content).map_err(|e| {
        logger::warn(&format!("[tool-cmd] write failed {}: {}", path.display(), e));
        e.to_string()
    })
}
